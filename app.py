...
model = pickle.load(open("logreg_model.pkl", "rb"))  ##loading model
...
def predict():
    ...
    row_df = pd.DataFrame([pd.Series(
        [text1, text2, text3, text4, text5, text6, text7, text8])])  ### Creating a dataframe using all the values
    print(row_df)
    prediction = model.predict_proba(row_df)  ## Predicting the output
    prediction=prediction*100
    output = '{0:.{1}f}'.format(prediction[0][1], 1)  ## Formating output

    if output > str(50):
        return render_template('index.html', pred='Você tem chances de ter diabetes.'
      '\n A Probabilidade de ter Diabetes é de {}%'.format(output))  ## Returning the message for use on the same index.html page
    else:
        return render_template('index.html', pred='Você não tem chances de ter diabetes.'
    '\n A Probabilidade de não ter Diabetes é de {}%%'.format(output))
